...
[snhwang/bear](https://github.com/snhwang/bear), pinned to `v0.1.7`.
...
requirements.txt            # bear@v0.1.7 + numerics + FastAPI for live-sim runs
```
...
(at the same v0.1.7 tag pinned in `requirements.txt`) so the live-sim
...
Part A is deterministic against the frozen bear `v0.1.7` and should reproduce
...
Pinned to bear `v0.1.7` (commit `8f9ff16`). Bumping bear will likely change
